...
loop3:
        ...
        addi s11, s11, 384 # B[k+1][j]
        addi t3, t3, 1 # k++ 
        blt t3, t0, loop3 # k<size , continue
        # loop3 end #
        sw t4, 0(a2) # store back to C[i][j]
        addi a2, a2, 4 # C[i][j+1]
        addi a0, a0, -384 # A go back
        addi a1, a1, 4 # B[k][j+1]
        add  s11, zero, a1 # B
        addi t2, t2, 1 # j++
        blt t2, t0, loop2 # j<size , continue
        # loop2 end #
        addi a0, a0, 384 # A[i+1][k]
        addi a1, a1, -384 # B go back (because line 40 add 2*128=256) AQUI SON 12 PORQUE 3 ELEMENTOS * 4 BYTES
        add s11, zero, a1 # B
        addi t1, t1, 1 # i++
